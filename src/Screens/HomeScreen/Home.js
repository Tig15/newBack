...
const Home = ({navigation}) => {
  ...
  useEffect(() => {
    setIsLoading(reqCatData.loading);
  }, [reqCatData]);
  ...
  const sliderData = processedData[0]; // [1] for Laraback
  const slides = sliderData.slides;

  const featuredStoresData = processedData[4];

  const defaultStore = featuredStoresData.categories.map(
    storeData => storeData.stores,
  );

  const popularStoresData = processedData[1];

  // const topOffersData = processedData[5];

  // const topDealsData = processedData[6];

  const bestDealData = processedData[2];

  const popDealData = processedData[3];
      ...
      {isLoading ? (
        ...
        <>
          ...
          <View style={homeStyle.secondContainer}>
            ...
            />
            <StoreCard
              data={selectedCategory ? selectedCategory.stores : defaultStore}
            />
            <FeatHead
            ...
            />
            <StoreCard
              data={popularStoresData.categories.map(
                storeData => storeData.stores,
              )}
            />
          </View>
